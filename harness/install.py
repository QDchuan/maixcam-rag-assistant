"""把 `harness/dsh-maixcam-shell` 装进一个 dsh profile。

这个包是「壳」：它不改 DeepSeek Harness 的任何东西，只是在某个 profile 的
插件目录里放一个包，再往那个 profile 的 `cordis.patch.yml` 里插一行。

装完之后**重启 dsh web**（或让 profile 的补丁层热重载生效）就能在侧栏看到
MaixCAM 的品牌。
...
用法：
    ...
    python -m harness.install --uninstall  # 撤回：删包目录 + 摘掉那一行
...
def cmd_check(profile: str) -> int:
    ...
    if (target / "lib" / "client.js").is_file():
        size = (target / "lib" / "client.js").stat().st_size
        print(f"  client.js : {size} 字节")
    if (target / "lib" / "index.js").is_file():
        size = (target / "lib" / "index.js").stat().st_size
        print(f"  index.js  : {size} 字节")
    ...
    ok = target.is_dir() and BEGIN in text
    print()
    print("状态：已装" if ok else "状态：未装")
