...
export function apply(ctx, config) {
	...
	async function search(query, k, aspects = []) {
		...
		if (aspects.length === 0) {
			// 基准查询用**按名次**取前 N 篇，**不套阈值**。
			//
			// 踩过的 bug：0.65 那个阈值是拿「面向查询」标定的（「舵机 PWM 控制」这类短词
			// 跟文档很像，余弦 0.73~0.81）。而整句自然语言问题（「寻找色块」）跟任何单片
			// 文档都不像，余弦基本都在 0.65 以下 —— 把同一个阈值套上去，基准列表直接被
			// 砍到 1 篇，面板看起来就是坏的。
			//
			// 两种语义本来就不同：
			//   基准查询 = 「哪些文章最相关」 → 按名次取前 N 篇
			//   面向查询 = 「这个子系统有没有证据」 → 过阈值，过线的全给
			const { docs, embedMs } = await sweep(c, query, { floor: 0, cap: 12 })
			const hits = docs
